public class CodeTest028PaintJob {
    public static void main(String[] args) {
        System.out.println(getBucketCount(5.6,6.5,3,1));
        System.out.println(getBucketCount(5.6,6.5,3));
    }
    public static int getBucketCount(double width, double height , double areaPerBucket, int extraBuckets ){
        if (width<=0||height<=0||areaPerBucket<=0||extraBuckets<0)  return -1;
            ...
            if (bucketCount>(int)bucketCount){
                return (int)(bucketCount+1-extraBuckets);
            }else  return (int)(bucketCount-extraBuckets);
        }
    }
    public static int getBucketCount(double width, double height , double areaPerBucket){
        if (width<=0||height<=0||areaPerBucket<=0)  return -1;
        else{
            double area=height*width, bucketCount=area/areaPerBucket;
            if(bucketCount>(int)bucketCount){
                return (int)(bucketCount+1);
            }else return (int)bucketCount;
        }
    }
    public static int getBucketCount(double area , double areaPerBucket){
        if (areaPerBucket<=0||area<=0) return -1;
            ...
            double bucketCount=area/areaPerBucket;
            if (bucketCount>(int)bucketCount) return (int)(bucketCount+1);
            else return (int)bucketCount;
        }
    }
}
...
//
//If one of the parameters width, height or areaPerBucket is less or equal to 0 or if extraBuckets is less than 0, the method
...
//
//*getBucketCount(2.75, 3.25, 2.5, 1); → should return 3 since the wall area is 8.9375, a single bucket can cover an area of
// 2.5 and Bob has 1 extra bucket at home.
//
//
//
